        ...
        <Card className="border-none shadow-sm">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <Badge
                variant={allStudied ? "default" : "secondary"}
                className={allStudied ? "bg-emerald-600" : ""}
              >
                {allStudied
                  ? "Модуль завершён!"
            ...
            <Progress
              value={(studiedCount / totalCount) * 100}
              className="h-2"
            />
          </CardContent>
        </Card>
          ...
          <CardContent className="p-5">
              ...
              {owaspItems.map((owaspItem) => (
                <div
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium text-white ${owaspItem.severityColor} cursor-pointer hover:opacity-80 transition-opacity ${
                    studiedOwaspItems.includes(owaspItem.id)
            ...
            <div className="flex gap-4 mt-3 text-[11px] text-muted-foreground">
              <span className="flex items-center gap-1">
                <span className="w-2.5 h-2.5 rounded bg-red-500" /> Критический
              </span>
              <span className="flex items-center gap-1">
                <span className="w-2.5 h-2.5 rounded bg-orange-500" /> Высокий
              </span>
              <span className="flex items-center gap-1">
                <span className="w-2.5 h-2.5 rounded bg-yellow-500" /> Средний
              </span>
            </div>
          </CardContent>
        </Card>
                    ...
                    <CardContent className="p-4 flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <Badge
                            className={`text-[10px] text-white ${owaspItem.severityColor} border-0`}
                          >
                            {owaspItem.severity}
                          </Badge>
                        </div>
        ...
        <Card className="border-none shadow-sm bg-gradient-to-br from-emerald-600 to-teal-700 text-white">
          <CardContent className="p-8 text-center">
            ...
            <h2 className="text-2xl font-bold mb-2">
              {allStudied
                ? "Все уроки пройдены!"
                : `${studiedCount} из ${totalCount} изучено`}
            </h2>
            ...
            <div className="flex gap-3 justify-center">
              ...
              {!isStudied && (
                <Button
                  className="bg-card text-emerald-700 hover:bg-emerald-50"
              ...
              {owaspItems.map((owaspItem) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`flex-1 ${done ? "text-emerald-700 font-medium" : "text-muted-foreground"}`}
                    >
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div
          className={`w-10 h-10 rounded-xl flex items-center justify-center ${item.severityColor} text-white`}
        >
          <Shield size={20} />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <Badge
              className={`text-[10px] text-white ${item.severityColor} border-0`}
            >
              {item.severity}
            </Badge>
          </div>
      ...
      <Card className="border-none shadow-sm">
        <CardContent className="p-4">
          ...
          <Progress
            value={
              ((currentLesson * steps.length + currentStepIndex + 1) /
              ...
              <CardContent className="p-5 space-y-4">
                ...
                <div className="bg-emerald-50 rounded-lg p-3 border border-emerald-200">
                  <p className="text-xs text-emerald-700">
                    <strong>Почему это важно:</strong> Эта уязвимость входит в
                    OWASP Top 10 — список наиболее критических угроз для
                    веб-приложений. Знание и понимание этой категории необходимо
            ...
            <Card className="border-border">
              <CardContent className="p-5 space-y-4">
                ...
                <div className="bg-secondary rounded-lg p-3">
                  <p className="text-xs text-muted-foreground">
                    <strong>Вывод:</strong> Реальные инциденты показывают, что
                    даже крупные компании с большими бюджетами на безопасность
                    подвержены этим уязвимостям. Понимание механизмов атак —
                    первый шаг к защите.
                  </p>
                </div>
              </CardContent>
              ...
              <CardContent className="p-5 space-y-4">
                ...
                <div className="bg-red-50 rounded-lg p-3 border border-red-200">
                  <p className="text-xs text-red-700">
                    <strong>Внимание:</strong> Приведённый код содержит
                    уязвимости. Не используйте подобные паттерны в продакшене.
                    Каждая строка с комментарием «УЯЗВИМЫЙ КОД» демонстрирует
                    типичную ошибку разработчика.
                  </p>
                </div>
              </CardContent>
            </Card>
          ...
          {lessonStep === "secure" && (
            <Card className="border-border">
              <CardContent className="p-5 space-y-4">
                ...
                <CodeBlock
                  code={item.secureCode}
                  language="javascript"
                  title="secure.js"
                />
                <div className="bg-emerald-50 rounded-lg p-3 border border-emerald-200">
                  <p className="text-xs text-emerald-700">
                    <strong>Обратите внимание:</strong> Безопасный код всегда
                    включает валидацию входных данных, проверку прав доступа и
                    обработку ошибок. Разница между уязвимым и безопасным кодом
                    часто заключается в нескольких строках проверок.
                  </p>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-5 space-y-4">
                ...
                <div className="bg-sky-50 rounded-lg p-3 border border-sky-200">
                  <p className="text-xs text-sky-700">
                    <strong>Совет:</strong> Применяйте эти рекомендации
                    комплексно. Один метод защиты редко бывает достаточен —
                    используйте «глубокую защиту» (Defense in Depth).
                  </p>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-5 space-y-4">
                ...
                <div className="bg-violet-50 rounded-lg p-3 border border-violet-200">
                  <p className="text-xs text-violet-700">
                    <strong>Совет:</strong> Используйте эти ресурсы для
                    углублённого изучения темы и практического тестирования на
      ...
      <div className="flex items-center justify-between">
        ...
        >
          {lessonStep === "mitigations" &&
          currentLesson === owaspItems.length - 1
            ? "Завершить модуль"
            : "Далее"}{" "}
