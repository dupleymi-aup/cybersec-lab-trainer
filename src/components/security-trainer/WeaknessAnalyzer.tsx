      ...
      .then(([modulePerf, quizCategoryData]) => {
          ...
          if (mod.completionRate < 60 || mod.avgScore < 60) {
            ...
            if (mod.completionRate < 40) {
              recommendations.push(
                `Повысить видимость модуля "${mod.moduleName}" в учебной программе`,
              );
            }
            if (mod.avgScore < 50) {
              recommendations.push(
                `Добавить дополнительные материалы и практику по "${mod.moduleName}"`,
              );
            }
            if (mod.difficultyIndex > 0.7) {
              recommendations.push(
                `Упростить или разбить на подуровни модуль "${mod.moduleName}"`,
              );
            }
            if (recommendations.length === 0) {
              recommendations.push(
                `Увеличить количество практических заданий в "${mod.moduleName}"`,
              );
            }
        ...
        for (const cat of quizStats) {
          if (cat.avgScore < 60) {
            ...
            if (cat.avgScore < 40) {
              recommendations.push(
                `Провести дополнительное занятие по "${cat.categoryName}"`,
              );
            }
            if (cat.questionStats.some((q) => q.correctRate < 0.3)) {
              recommendations.push(
                `Пересмотреть сложные вопросы в категории "${cat.categoryName}"`,
              );
            }
            if (recommendations.length === 0) {
              recommendations.push(
                `Добавить больше практики по "${cat.categoryName}"`,
              );
            }
              ...
              <button
                ...
                className={`p-3 rounded-lg text-center transition-all ${
                  selectedSeverity === "all"
                    ? "bg-slate-800 dark:bg-slate-700 text-white ring-2 ring-slate-800"
                ...
                >
                  ...
                  <Bar dataKey="score" radius={[0, 4, 4, 0]}>
                    {chartData.map((entry, i) => (
                      <Cell
                        key={i}
                        fill={SEVERITY_CONFIG[entry.severity].color}
                      />
                    ))}
                  </Bar>
                </BarChart>
