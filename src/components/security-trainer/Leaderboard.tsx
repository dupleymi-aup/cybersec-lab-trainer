...
const generateSimulatedStudents = (currentUserId: string): StudentStats[] => {
  const students = [
    { id: 'usr_1', name: 'Алексей Петров', group: 'ИС-21', modulesCompleted: 10, quizAvg: 92, achievements: 18, streak: 15, lastActive: '2026-05-19' },
    { id: 'usr_2', name: 'Мария Иванова', group: 'ИС-21', modulesCompleted: 9, quizAvg: 88, achievements: 16, streak: 12, lastActive: '2026-05-18' },
    { id: 'usr_3', name: 'Дмитрий Козлов', group: 'ИС-22', modulesCompleted: 8, quizAvg: 85, achievements: 14, streak: 8, lastActive: '2026-05-17' },
    { id: 'usr_4', name: 'Анна Смирнова', group: 'ИС-21', modulesCompleted: 8, quizAvg: 78, achievements: 12, streak: 5, lastActive: '2026-05-19' },
    { id: 'usr_5', name: 'Игорь Волков', group: 'ИС-22', modulesCompleted: 7, quizAvg: 82, achievements: 13, streak: 10, lastActive: '2026-05-18' },
    { id: 'usr_6', name: 'Елена Новикова', group: 'ИС-21', modulesCompleted: 6, quizAvg: 75, achievements: 10, streak: 3, lastActive: '2026-05-16' },
    { id: 'usr_7', name: 'Сергей Морозов', group: 'ИС-22', modulesCompleted: 6, quizAvg: 70, achievements: 9, streak: 2, lastActive: '2026-05-15' },
    { id: 'usr_8', name: 'Ольга Соколова', group: 'ИС-21', modulesCompleted: 5, quizAvg: 68, achievements: 8, streak: 4, lastActive: '2026-05-19' },
    { id: 'usr_9', name: 'Андрей Лебедев', group: 'ИС-22', modulesCompleted: 4, quizAvg: 62, achievements: 6, streak: 1, lastActive: '2026-05-14' },
...
export default function Leaderboard() {
  ...
  const allStudents = useMemo(() => {
    ...
    const avgQuiz = quizAvgValues.length > 0
      ? Math.round(quizAvgValues.reduce((a, b) => a + b, 0) / quizAvgValues.length)
      : 0;
    ...
    return students.map(s => {
      if (s.id === (user?.id || 'current')) {
        return {
          ...
          quizAvg: avgQuiz,
          xp: calculateXP({ ...s, modulesCompleted: completedModules.length, quizAvg: avgQuiz }),
        };
      }
  ...
  const sortedStudents = useMemo(() => {
    return [...allStudents].sort((a, b) => {
      switch (sortBy) {
        ...
        case 'modules': return b.modulesCompleted - a.modulesCompleted;
        case 'quiz': return b.quizAvg - a.quizAvg;
      ...
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Рейтинг студентов по прогрессу обучения</p>
          </div>
        </div>
      </div>
          ...
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <h3 className="font-semibold text-foreground">Ваша статистика</h3>
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-4">
                  ...
                  <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0"
                    style={{ background: rank <= 3 ? medalColors[rank - 1] : 'transparent' }}
                  >
                    {rank <= 3 ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className={`font-semibold truncate ${isCurrentUser ? 'text-violet-700 dark:text-violet-300' : 'text-foreground'}`}>
                        {student.name}
                      </p>
                      {isCurrentUser && (
                    ...
                    <p className="text-xs text-muted-foreground">{student.group} • Активность: {student.lastActive}</p>
                  </div>

                  {/* Stats */}
                  <div className="hidden md:flex items-center gap-4 text-sm">
                    <div className="text-center">
                      <div className="font-semibold text-foreground">{student.modulesCompleted}</div>
                      <div className="text-xs text-muted-foreground">модулей</div>
                    </div>
                    <div className="text-center">
                      <div className="font-semibold text-foreground">{student.quizAvg}%</div>
      ...
      <Card className="mt-6 border-border/50">
        <CardContent className="p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { label: 'Модуль', value: '+100 XP', desc: 'За каждый пройденный модуль' },
              { label: 'Квиз', value: '+5 XP', desc: 'За 1% результата квиза' },
              { label: 'Достижение', value: '+50 XP', desc: 'За каждое достижение' },
              { label: 'Стрик', value: '+10 XP', desc: 'За каждый день стрика' },
            ].map((item, i) => (
