...
import { securityHeaders } from '@/lib/security-data';
import CodeBlock from './CodeBlock';
import { Card, CardContent } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { motion, AnimatePresence } from 'framer-motion';
import { ChevronLeft, CheckCircle2, ArrowRight, ArrowLeft, ShieldCheck, AlertTriangle, Lightbulb, Trophy } from 'lucide-react';
...
const categoryColors: Record<string, string> = {
  ...
  'Контроль фреймов': 'bg-amber-100 text-amber-700',
  'Типы контента': 'bg-purple-100 text-purple-700',
  'Конфиденциальность': 'bg-indigo-100 text-indigo-700',
  'Разрешения браузера': 'bg-teal-100 text-teal-700',
};
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1">
        {stepLabels.map((s, i) => (
          <button
            key={s.key}
            onClick={() => { setStep(s.key); setQuizAnswer(null); setQuizSubmitted(false); }}
          ...
          {step === 'description' && (
            <Card className="border-sky-200 bg-sky-50">
              <CardContent className="p-5 space-y-4">
                ...
                <p className="text-sm text-sky-700 leading-relaxed">{header.description}</p>
              </CardContent>
            </Card>
          )}
          ...
          {step === 'attack' && (
            <Card className="border-red-200 bg-red-50">
              <CardContent className="p-5 space-y-4">
                ...
                <p className="text-sm text-red-700 leading-relaxed">{header.attackDemo}</p>
              </CardContent>
            </Card>
          )}
