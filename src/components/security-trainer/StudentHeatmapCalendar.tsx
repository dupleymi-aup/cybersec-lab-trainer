...
export default function StudentHeatmapCalendar({
  groupId: controlledGroupId,
}: { groupId?: string } = {}) {
  const [students, setStudents] = useState<
    Array<{ id: string; fullName: string; group: string }>
  >([]);
  ...
  useEffect(() => {
    const loadStudents = async () => {
      ...
      setStudents(
        controlledGroupId
          ? mapped.filter((s) => s.group === controlledGroupId)
          : mapped,
      );
    };
    loadStudents();
  ...
  const weeks: Array<
    Array<{ date: string; dayOfWeek: number; count: number }>
  > = [];
  const now = new Date();
    ...
    <div className="space-y-6">
      ...
      {!loading && !error && selectedStudentId && (
        <>
            ...
            <CardContent className="p-5">
              ...
              <div className="flex gap-1 mb-1 ml-8">
                {weeks[0]?.map((d, i) => (
                  <div
                    key={i}
                    className="w-4 text-[8px] text-muted-foreground text-center"
                  />
                ))}
              </div>
              ...
              <div className="flex gap-1">
                ...
                <div className="flex flex-col gap-1 mr-2">
                  {DAY_LABELS.map((label, i) => (
                    <div
                      key={i}
                      className="h-4 flex items-center text-[9px] text-muted-foreground w-6"
                    >
                      {label}
                    </div>
                  ))}
          ...
          <Card className="border-border">
            <CardContent className="p-5">
              <h3 className="font-semibold text-sm mb-4">
                Детали активности (последние 7 дней)
              </h3>
              <div className="grid grid-cols-7 gap-2">
                {Array.from({ length: 7 }).map((_, i) => {
                  ...
                  return (
                    <div
                      key={dateKey}
                      className={`p-3 rounded-lg border text-center ${count > 0 ? "border-indigo-200 bg-indigo-50/50" : "border-border"}`}
                    >
                      <p className="text-xs text-muted-foreground">{dayName}</p>
                      <p className="text-lg font-bold">{dayNum}</p>
                      <p
                        className={`text-sm font-semibold ${count > 0 ? "text-indigo-600" : "text-muted-foreground"}`}
                      >
                        {count}
                      </p>
                    </div>
      ...
      {!selectedStudentId && !loading && (
        <div className="flex items-center justify-center py-16">
          ...
          <p className="text-sm text-muted-foreground ml-3">
            Выберите студента для просмотра календаря активности
          </p>
        </div>
      )}
    </div>
