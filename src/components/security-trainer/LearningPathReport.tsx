...
const isControlled = (
  props: LearningPathReportProps,
): props is LearningPathReportProps & { days: number } =>
...
export default function LearningPathReport(
  ...
  const dropOffRates = path.map((entry, i) => {
    ...
    return (
      Math.round(
        ((prevPercentage - entry.percentage) / prevPercentage) * 10000,
      ) / 100
    );
  });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <KPICard
          ...
          value={
            path.length > 0 ? `${path[path.length - 1].percentage}%` : "0%"
          }
