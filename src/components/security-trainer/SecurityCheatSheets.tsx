    ...
    items: [
      {
        ...
        content:
          "Проверяйте авторизацию на каждом эндпоинте. Используйте deny-by-default.",
        code: "if (resource.userId !== req.user.id) return res.status(403);",
      },
      {
        title: "A02 — Cryptographic Failures",
        content:
          "Используйте bcrypt для паролей, TLS 1.3 для передачи, AES-256 для хранения.",
        code: "const hash = await bcrypt.hash(password, 12);",
      },
      {
        title: "A03 — Injection",
        content:
          "Используйте параметризованные запросы. Никогда не конкатенируйте SQL.",
        code: 'db.query("SELECT * FROM users WHERE id = $1", [userId]);',
      },
      {
        title: "A04 — Insecure Design",
        content:
          "Threat modeling на этапе проектирования. Rate limiting, CAPTCHA, business logic validation.",
        code: "// Rate limiting\nconst limiter = rateLimit({ windowMs: 15*60*1000, max: 100 });",
      },
      {
        title: "A05 — Security Misconfiguration",
        content:
          "Отключите debug mode в production. Security headers. Обновляйте зависимости.",
        code: 'app.use(helmet());\napp.disable("x-powered-by");',
      },
      {
        title: "A06 — Vulnerable Components",
        content:
          "Аудит зависимостей: npm audit, Snyk, Dependabot. Lock-файлы обязательны.",
        code: "npm audit fix\nnpx depcheck",
      },
      {
        title: "A07 — Auth Failures",
        content:
          "MFA обязательна. Короткоживущие сессии. Secure password reset tokens.",
        code: 'jwt.sign(payload, secret, { expiresIn: "1h", algorithm: "HS256" });',
      },
      {
        title: "A08 — Data Integrity",
        content:
          "SRI для CDN-ресурсов. Цифровые подписи. CI/CD pipeline security.",
        code: '<script src="cdn.js" integrity="sha384-..." crossorigin="anonymous">',
      },
      {
        title: "A09 — Logging Failures",
        content:
          "Логируйте auth events, ошибки, access. Никогда не логируйте пароли и токены.",
        code: "console.error(err); // НЕ console.error(password, token);",
      },
      {
        title: "A10 — SSRF",
        content:
          "Allowlist доменов. Блокируйте private IPs. Валидируйте URL после redirect.",
    ...
    items: [
      {
        ...
        content:
          "Золотой стандарт защиты. Данные передаются отдельно от SQL-кода.",
    ...
    items: [
      {
        ...
        content:
          "Экранируйте HTML-сущности при выводе пользовательских данных.",
      ...
      },
      {
        ...
        content:
          "React автоматически экранирует JSX, но опасен dangerouslySetInnerHTML.",
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        content:
          "Криптографически случайный токен, 1 час expiry, однократное использование.",
        code: 'const token = crypto.randomBytes(32).toString("hex");\nconst expires = Date.now() + 3600000; // 1 hour',
      },
      {
        title: "Session Security",
        content:
          "Secure, HttpOnly, SameSite cookies. Регенерация после логина.",
...
export default function SecurityCheatSheets() {
  ...
  const filteredSheets = cheatSheets.filter((sheet) => {
    if (activeCategory !== "all" && sheet.category !== activeCategory)
      return false;
    if (searchQuery) {
    ...
    >
      ...
      <div className="mb-6">
        ...
        <p className="text-muted-foreground text-sm">
          Быстрый справочник по основным практикам безопасности: OWASP Top 10,
          security headers, защита от инъекций, криптография, сетевая
          безопасность.
        </p>
      </div>
          ...
          <Card key={sheet.id} className="border-border/50">
            <CardContent className="p-5">
              <div className="flex items-center gap-3 mb-4">
                ...
                <Badge className={categoryColors[sheet.category]}>
                  {sheet.category}
                </Badge>
                    ...
                    {item.code && (
                      <div className="relative">
                        ...
                        <button
                          onClick={() =>
                            item.code &&
                            handleCopy(item.code, `${sheet.id}-${idx}`)
                          }
