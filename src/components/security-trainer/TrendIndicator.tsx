...
export default function TrendIndicator({ trend, value, suffix = '', size = 'sm' }: TrendIndicatorProps) {
  const normalizedTrend = trend === 'improving' ? 'up' : trend === 'declining' ? 'down' : trend;

  const iconSize = size === 'sm' ? 12 : 16;

  const icon = normalizedTrend === 'up' ? (
  ...
  const textColor = normalizedTrend === 'up' ? 'text-emerald-600' : normalizedTrend === 'down' ? 'text-red-600' : 'text-slate-400';
