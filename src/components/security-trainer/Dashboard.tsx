...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    getAuthHeaders()
      .then((headers) => fetch('/api/deadlines/upcoming', { headers, signal: controller.signal }))
      .then((r) => r.json())
      .then((data) => {
        if (data.upcoming) {
      ...
      .catch((err) => {
        if (err.name !== 'AbortError') logger.error('Dashboard failed to load notifications', { error: err });
      });
  ...
  useEffect(() => {
    ...
    getAuthHeaders()
      ...
      .catch((err) => {
        if (err.name !== 'AbortError') logger.warn('Dashboard failed to load teacher stats', { error: err });
      });
    return () => controller.abort();
  }, [user?.role]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 md:hidden">
        ...
        <span className="text-lg font-bold">CyberSec Lab</span>
                  ...
                  <div className="flex flex-1 items-start gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="text-foreground/80 text-sm font-semibold">{ann.title}</p>
                        {ann.priority === 'high' && (
                      ...
                      <p className="text-muted-foreground mt-0.5 text-xs">{ann.content}</p>
                    </div>
                  </div>
                  <button
                    onClick={() => dismissAnnouncement(ann.id)}
                    className="hover:text-muted-foreground shrink-0 text-slate-400"
      ...
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 via-slate-800 to-emerald-900 p-6 text-white md:p-10"
      >
        <div className="absolute top-0 right-0 h-64 w-64 translate-x-1/2 -translate-y-1/2 rounded-full bg-emerald-500/10 blur-3xl" />
        <div className="absolute bottom-0 left-0 h-48 w-48 -translate-x-1/2 translate-y-1/2 rounded-full bg-amber-500/5 blur-3xl" />
        <div className="relative z-10">
          <div className="mb-3 flex flex-wrap items-center gap-2">
            <Badge className="border-emerald-600/30 bg-emerald-600/30 text-emerald-300">{t('heroBadge')}</Badge>
          ...
          <p className="max-w-2xl text-sm leading-relaxed text-slate-300 md:text-base">{t('subtitle')}</p>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:gap-4 lg:grid-cols-6">
        {[
          ...
          },
          {
            ...
            value: streakData.current > 0 ? `${streakData.current} ${t('days')}` : '—',
            color: 'text-orange-600',
            tooltip: streakData.longest > 0 ? t('stats.bestStreak', { streak: streakData.longest }) : undefined,
            streakKey: true,
          },
          {
            label: t('level'),
          ...
          >
            <Card className="bg-card border-none shadow-sm transition-shadow hover:shadow-md">
                ...
                {'streakKey' in stat ? (
                  <>
                    ...
                    {stat.tooltip && streakData.longest > 0 && (
                      <p className="mt-0.5 text-[10px] text-orange-500">{stat.tooltip}</p>
                    )}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          <Card
            className="h-full cursor-pointer border-emerald-200 bg-gradient-to-r from-emerald-50 to-teal-50 transition-shadow hover:shadow-md"
          ...
          >
            <CardContent className="flex items-center justify-between gap-4 p-5">
              <div className="flex flex-1 items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-medium text-emerald-600">{t('recommendation')}</p>
                  <p className="text-foreground/80 text-sm font-semibold">{recommendation.text}</p>
                </div>
              </div>
        ...
        {nextAchievement && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}>
            <Card className="h-full border-amber-200 bg-gradient-to-r from-amber-50 to-yellow-50">
              ...
              <div className="space-y-3">
                {upcomingDeadlines.map((deadline) => {
                  ...
                  const urgencyBadge = deadline.isOverdue ? (
                    <Badge className="border-0 bg-red-100 text-[10px] text-red-700">{t('deadline.overdue')}</Badge>
                  ) : deadline.daysLeft <= 1 ? (
                    <Badge className="border-0 bg-red-100 text-[10px] text-red-700">
                      {deadline.daysLeft === 0 ? t('deadline.today') : t('deadline.tomorrow')}
                    </Badge>
                  ) : (
                    <Badge className="border-0 bg-orange-100 text-[10px] text-orange-700">
                  ...
                  return (
                    <div
                      key={deadline.id}
                      className={`rounded-lg border-l-4 ${urgencyColor} cursor-pointer p-4 transition-shadow hover:shadow-md`}
                      onClick={() => setCurrentPage(targetPage)}
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          <div className="mb-1 flex items-center gap-2">
                            <p className="text-foreground/80 truncate text-sm font-semibold">{deadline.title}</p>
                            {urgencyBadge}
                          </div>
                          <p className="text-muted-foreground text-xs">{scopeLabel}</p>
                          {deadline.description && (
                            <p className="mt-1 text-xs text-slate-400">{deadline.description}</p>
                          )}
                        </div>
                        <div className="shrink-0 text-right">
                          <p className="text-muted-foreground text-xs">{dueDate}</p>
                          <ChevronRight size={14} className="mt-1 ml-auto text-slate-300" />
                        </div>
                      </div>
                    </div>
                  );
      ...
      {/* Module Cards */}
      <div>
        <h2 className="mb-4 text-lg font-bold md:text-xl">{t('learningModules')}</h2>
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          {modules.map((mod, i) => {
            ...
            return (
              ...
              >
                <Card
                  className="group border-border h-full cursor-pointer overflow-hidden transition-all duration-300 hover:border-emerald-300 hover:shadow-md"
                  onClick={() => handleStartModule(mod.id)}
                >
                  <CardContent className="p-0">
                    <div className="flex">
                      <div
                        className={`flex w-16 shrink-0 items-center justify-center md:w-20 ${
                          isCompleted ? 'bg-emerald-50' : 'bg-secondary'
                        }`}
                      >
                        <span className={isCompleted ? 'text-emerald-600' : 'text-slate-400'}>{iconMap[mod.icon]}</span>
                      </div>
                    ...
                    {(() => {
                      ...
                      return pct > 0 || isCompleted ? (
                        <div className="px-4 pt-1 pb-3">
                          ...
                          {label && <p className="mt-1 text-[10px] text-slate-400">{label}</p>}
                        </div>
                      ) : null;
                    })()}
                  </CardContent>
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          >
            <Card
              className="group h-full cursor-pointer overflow-hidden border-amber-200 transition-all duration-300 hover:border-amber-400 hover:shadow-md"
              onClick={() => setCurrentPage('quiz')}
            >
              <CardContent className="p-0">
                <div className="flex">
                  <div className="flex w-16 shrink-0 items-center justify-center bg-amber-50 md:w-20">
                  ...
                  <div className="flex-1 p-4">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <h3 className="text-sm font-semibold transition-colors group-hover:text-amber-700">
                          {t('quizCard.title')}
                        </h3>
                        <p className="text-muted-foreground mt-1 line-clamp-2 text-xs leading-relaxed">
          ...
          >
            <Card
              className="group h-full cursor-pointer overflow-hidden border-violet-200 transition-all duration-300 hover:border-violet-400 hover:shadow-md"
              onClick={() => setCurrentPage('achievements')}
            >
              <CardContent className="p-0">
                <div className="flex">
                  <div className="flex w-16 shrink-0 items-center justify-center bg-violet-50 md:w-20">
                  ...
                  <div className="flex-1 p-4">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <h3 className="text-sm font-semibold transition-colors group-hover:text-violet-700">
                      ...
                      <ChevronRight
                        size={16}
                        className="mt-1 shrink-0 text-slate-300 transition-colors group-hover:text-violet-500"
                      />
                    </div>
                    <div className="mt-3 flex flex-wrap items-center gap-2">
                      <Badge className="border-0 bg-violet-100 text-[10px] text-violet-700">
      ...
      <Card className="bg-card border-none shadow-sm">
        <CardContent className="p-6">
          <h3 className="mb-3 text-sm font-semibold">{t('overallProgress')}</h3>
          <Progress value={totalProgress} className="mb-2 h-3" />
