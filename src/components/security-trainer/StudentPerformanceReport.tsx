  ...
  if (!userId) {
    return (
      <div className="flex items-center justify-center py-16">
        <p className="text-sm text-muted-foreground">Выберите студента для просмотра отчёта</p>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="flex items-center justify-center py-16">
        ...
        <p className="text-sm text-muted-foreground font-medium ml-3">Ошибка загрузки</p>
      </div>
    );
  }
          ...
          <Card className="border-border">
            <CardContent className="p-5">
              ...
              {quizResults.length > 0 ? (
                ...
                <p className="text-sm text-muted-foreground text-center py-8">Нет результатов квизов</p>
              )}
            </CardContent>
          </Card>
        </TabsContent>
