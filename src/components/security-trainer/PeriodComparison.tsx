...
export default function PeriodComparison({ groupId }: { groupId?: string }) {
  ...
  const kpiComparisons = [
    ...
    },
    {
      ...
      delta: computeDelta(
        kpisA.avgCompletionRate,
        kpisB.avgCompletionRate,
        "%",
      ),
    },
    {
    ...
    },
    {
      ...
      delta: computeDelta(
        kpisA.totalModulesCompleted,
        kpisB.totalModulesCompleted,
      ),
    },
    {
      label: "Вовлечённость",
  ...
  const sortedByGain = [...moduleComparison].sort(
    (a, b) => b.periodA - b.periodB - (a.periodA - a.periodB),
  );
  const biggestGains = sortedByGain
    .filter((m) => m.periodA - m.periodB > 0)
    .slice(0, 3);
  const biggestLosses = sortedByGain
    .filter((m) => m.periodA - m.periodB < 0)
    .slice(0, 3);
    ...
    <div className="space-y-4">
      ...
      <Card className="border-border">
        <CardContent className="p-5">
          ...
          {(() => {
            const positiveDeltas = kpiComparisons.filter(
              (k) => k.delta.direction === "up",
            ).length;
            const negativeDeltas = kpiComparisons.filter(
              (k) => k.delta.direction === "down",
            ).length;
            const stableDeltas = kpiComparisons.filter(
              (k) => k.delta.direction === "stable",
            ).length;
            const trend =
              positiveDeltas > negativeDeltas
                ? "improving"
            ...
            return (
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {positiveDeltas} показателей улучшилось, {negativeDeltas}{" "}
                  снизилось, {stableDeltas} без изменений
                </p>
              </div>
            );
          })()}
