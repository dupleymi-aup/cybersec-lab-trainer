...
const iconColors = ["bg-violet-500/10 text-violet-500", "bg-emerald-500/10 text-emerald-500", "bg-amber-500/10 text-amber-500"];
      ...
      <div className="relative z-10 container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {items.map((review, index) => {
            ...
            return (
              <motion.div key={review.name} initial={{opacity: 0, y: 30}} whileInView={{opacity: 1, y: 0}} viewport={{once: true}} transition={{duration: 0.5, delay: index * 0.15}}>
                <div className="h-full p-6 rounded-2xl bg-card border border-border hover:border-border/80 transition-all group relative">
                  <Quote className="absolute top-4 right-4 w-8 h-8 text-muted/20" aria-hidden="true" />
                  <div className="flex items-center gap-4 mb-4">
                    <div className={"flex items-center justify-center w-12 h-12 rounded-full " + iconColors[index % iconColors.length] + " font-bold text-lg"}>{avatars[index]}</div>
                    <div>
                  ...
                  <div className="flex gap-1 mb-4" role="img" aria-label={"Rating: 5 out of 5"}>
                    {[...Array(5)].map((_, i) => (<Star key={i} className="w-4 h-4 fill-current text-amber-500" aria-hidden="true" />))}
                  </div>
                  ...
                  <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-accent/50 rounded-lg">
                    ...
                    <span className="text-xs text-muted-foreground">{review.achievement}</span>
                  </div>
                </div>
              </motion.div>
            );
