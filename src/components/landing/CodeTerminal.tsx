...
const codeLines = [
  ...
  {text: "  const { username, password } = req.body;", color: "text-foreground/80"},
  {text: "", color: ""},
...
export default function CodeTerminal() {
      ...
      <div className="w-full max-w-xl mx-auto" role="img" aria-label="Code terminal">
        <div className="rounded-xl overflow-hidden bg-card border border-border shadow-2xl shadow-violet-500/10">
          <div className="flex items-center justify-between px-4 py-3 bg-muted/50 border-b border-border">
            ...
            <div className="flex items-center gap-2 text-muted-foreground text-sm"><Terminal className="w-4 h-4" aria-hidden="true" /><span>vulnerability.js</span></div>
            <div className="flex items-center gap-1.5"><Minus className="w-4 h-4 text-muted-foreground" /><Square className="w-3.5 h-3.5 text-muted-foreground" /><X className="w-4 h-4 text-muted-foreground" /></div>
          </div>
          <div className="p-4 font-mono text-sm leading-relaxed min-h-[320px]" aria-hidden="true" />
        </div>
      </div>
    );
  }
  ...
  return (
    <motion.div initial={{opacity: 0, scale: 0.95}} whileInView={{opacity: 1, scale: 1}} viewport={{once: true}} transition={{duration: 0.6, delay: 0.3}} className="w-full max-w-xl mx-auto" role="img" aria-label="Animated code terminal">
      <div className="rounded-xl overflow-hidden bg-card border border-border shadow-2xl shadow-violet-500/10">
        <div className="flex items-center justify-between px-4 py-3 bg-muted/50 border-b border-border">
          <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-red-500/80" /><div className="w-3 h-3 rounded-full bg-amber-500/80" /><div className="w-3 h-3 rounded-full bg-emerald-500/80" /></div>
          <div className="flex items-center gap-2 text-muted-foreground text-sm"><Terminal className="w-4 h-4" aria-hidden="true" /><span>vulnerability.js</span></div>
        ...
        <div className="p-4 font-mono text-sm leading-relaxed min-h-[320px]">
          {codeLines.slice(0, displayedLines).map((line, index) => (
            <div key={index} className="flex">
              <span className="text-muted-foreground/40 w-8 mr-4 text-right select-none flex-shrink-0">{index + 1}</span>
              <span className={line.color || "text-foreground/80"}>
                {line.text === "// comment" ? t("comment") : line.text}
                {index === displayedLines - 1 && !isComplete && cursorVisible && (<span className="inline-block w-2 h-4 ml-0.5 bg-violet-500 animate-pulse" />)}
              </span>
            </div>
          ))}
        </div>
        {isComplete && (
          <motion.div initial={{opacity: 0}} animate={{opacity: 1}} className="mx-4 mb-4 p-3 bg-red-500/10 border border-red-500/20 rounded-lg" role="alert">
            <p className="text-red-500 text-xs font-semibold mb-1">{t("alert")}</p>
