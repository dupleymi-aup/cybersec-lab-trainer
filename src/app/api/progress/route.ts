...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    type ProgressRow = { moduleId: string; completed: boolean; score: number | null; sqlLevels: string | null; xssLevels: string | null; csrfSteps: string | null; csrfChallengeScores: string | null; secureCodingAnswers: string | null; secureCodingCorrectCount: number; studiedOwaspItems: string | null; challengeScores: string | null };
    return NextResponse.json({
      completedModules,
      quizScores,
      progress: progress.map((p: ProgressRow) => ({
        moduleId: p.moduleId,
    ...
    logger.error('progress GET error:', { error: String(e) });
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    logger.error('progress POST error:', { error: String(e) });
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
