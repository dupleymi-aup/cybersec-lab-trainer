...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!platform) {
      return NextResponse.json({ error: 'LTI platform not found or inactive' }, { status: 404 });
    }
    ...
    const isTeacher = roles.some((r) => r.toLowerCase().includes('instructor') || r.toLowerCase().includes('teacher'));
  ...
  try {
    ...
    for (const platform of platforms) {
      try {
        ...
        logger.error('Failed to parse public key to JWK', {
          error: String(e),
          platformId: platform.id,
        });
        // Skip invalid keys
