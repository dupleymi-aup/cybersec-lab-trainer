  ...
  try {
    ...
    const [
      ...
      getPrisma()
        ...
        .then((results: Array<{ userId: string | null; _count: number }>) => results.filter((r) => r.userId).length),
    ...
    // Build top actions
    const topActionsList = topActions.map((a: { action: string | null; _count: number }) => ({
      action: a.action ?? '',
      count: a._count,
    }));
