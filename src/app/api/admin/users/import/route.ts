...
export async function POST(request: NextRequest) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (!row.phone) {
      errors.push({ row: rowNum, email: row.email, error: 'Phone is required' });
      skipped++;
      continue;
    }
    if (!row.fullName) {
      errors.push({ row: rowNum, email: row.email, error: 'Full name is required' });
      skipped++;
      continue;
    }
    ...
    if (!validatePhone(row.phone)) {
      errors.push({ row: rowNum, email: row.email, error: 'Invalid phone format' });
      skipped++;
      continue;
    }
    ...
    if (seenEmails.has(row.email.toLowerCase())) {
      errors.push({ row: rowNum, email: row.email, error: 'Duplicate email in CSV' });
      skipped++;
      continue;
    }
    if (seenPhones.has(row.phone)) {
      errors.push({ row: rowNum, email: row.email, error: 'Duplicate phone in CSV' });
      skipped++;
      continue;
    }
  ...
  const existingUsers = await prisma.user.findMany({
    where: {
      OR: [
        ...
        { phone: { in: Array.from(seenPhones) } },
      ],
    },
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (existingPhones.has(row.phone)) {
      errors.push({ row: rowNum, email: row.email, error: 'Phone already exists' });
      skipped++;
      continue;
    }
