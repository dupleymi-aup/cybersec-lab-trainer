...
export async function GET(request: NextRequest) {
  ...
  const results: Array<{
    ...
    avgScore: number;
    completionRate: number;
    studiedItemsCoverage?: Array<{
      item: string;
      studiedCount: number;
      studiedRate: number;
    }>;
    secureCodingDistribution?: Array<{
      correctRange: string;
      min: number;
      max: number;
      count: number;
  ...
  for (const [moduleId, levelCount] of Object.entries(MODULE_LEVEL_COUNTS)) {
    ...
    for (let level = 1; level <= levelCount; level++) {
      ...
      for (const p of moduleProgress) {
        ...
        if (moduleId === "sql-injection")
          levelData = p.sqlLevels ? p.sqlLevels.split(",") : [];
        else if (moduleId === "xss")
          levelData = p.xssLevels ? p.xssLevels.split(",") : [];
          ...
          const answers = p.secureCodingAnswers
            ? p.secureCodingAnswers.split(",")
            : [];
      ...
      if (
        !["sql-injection", "xss", "csrf", "secure-coding"].includes(moduleId)
      ) {
        levels.push({
          level,
        ...
        levels.push({
          ...
          completionRate:
            totalStudents > 0
              ? Math.round((startedCount / totalStudents) * 10000) / 100
              : 0,
        });
      }
    }
    ...
    for (const p of moduleProgress) {
      ...
      let studiedItemsCoverage:
        | Array<{ item: string; studiedCount: number; studiedRate: number }>
        | undefined;
      if (moduleId === "owasp") {
        ...
        studiedItemsCoverage = Array.from(allItems).map((item) => ({
          ...
          studiedRate:
            totalStudents > 0
              ? Math.round(((itemCounts[item] || 0) / totalStudents) * 10000) /
                100
              : 0,
        }));
      }
      ...
      if (moduleId === "secure-coding") {
        ...
        for (const p of moduleProgress) {
          for (const r of ranges) {
            if (
              p.secureCodingCorrectCount >= r.min &&
              p.secureCodingCorrectCount <= r.max
            ) {
              r.count++;
              break;
            }
