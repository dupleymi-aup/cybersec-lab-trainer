...
export async function GET(request: NextRequest) {
  ...
  for (const student of students) {
    ...
    const totalDays = Math.max(
      1,
      Math.floor(
        (lastModuleDate.getTime() - firstModuleDate.getTime()) /
          (24 * 60 * 60 * 1000),
      ),
    );
    const avgDaysPerModule =
      Math.round((totalDays / modulesCompleted) * 10) / 10;
    ...
    const speedScore = Math.max(
      0,
      Math.min(100, Math.round((1 - avgDaysPerModule / 14) * 100)),
    );
    // Improvement score (capped at +50 improvement = 100)
    const improvementScore = Math.max(
      0,
      Math.min(100, Math.round(((scoreImprovement + 50) / 100) * 100)),
    );
  ...
  const groupMap = new Map<
    string,
    { days: number; improvement: number; count: number }
  >();
  for (const v of studentVelocities) {
  ...
  const avgVelocityByGroup = Array.from(groupMap.entries()).map(
    ([group, data]) => ({
      group,
      avgDaysPerModule: Math.round((data.days / data.count) * 10) / 10,
      avgScoreImprovement:
        Math.round((data.improvement / data.count) * 10) / 10,
    }),
  );
