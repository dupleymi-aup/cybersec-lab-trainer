...
export async function GET(request: NextRequest) {
  ...
  if (studentIds.length === 0) {
    return NextResponse.json({ daily: [], summary: { totalModulesCompleted: 0, totalQuizAttempts: 0, avgDailyActive: 0, trend: 'stable' as const } });
  }
  ...
  // Build daily data
  const dailyMap = new Map<string, { modulesCompleted: number; modulesStarted: number; quizAttempts: number; quizScores: number[]; activeStudents: Set<string>; newCompletions: number }>();

  for (let d = new Date(since); d <= now; d.setDate(d.getDate() + 1)) {
    const dateStr = d.toISOString().split('T')[0];
    dailyMap.set(dateStr, { modulesCompleted: 0, modulesStarted: 0, quizAttempts: 0, quizScores: [], activeStudents: new Set(), newCompletions: 0 });
  }
  ...
  const daily = Array.from(dailyMap.entries())
    ...
    .map(([date, data]) => ({
      ...
      avgQuizScore: data.quizScores.length > 0 ? Math.round(data.quizScores.reduce((a, b) => a + b, 0) / data.quizScores.length * 10) / 10 : 0,
  ...
  const avgDailyActive = daily.length > 0 ? Math.round(daily.reduce((sum, d) => sum + d.activeStudents, 0) / daily.length * 10) / 10 : 0;
