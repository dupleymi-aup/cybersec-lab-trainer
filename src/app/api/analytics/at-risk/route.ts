...
export async function GET(request: NextRequest) {
  ...
  const students = await prisma.user.findMany({
    where: userWhere,
    select: { id: true, fullName: true, email: true, group: true, course: true, university: true, lastLoginAt: true },
  });
  ...
  for (const student of students) {
    ...
    const avgQuizScore = studentQuizResults.length > 0
      ? Math.round(studentQuizResults.reduce((sum, q) => sum + q.percentage, 0) / studentQuizResults.length * 10) / 10
      : 0;
    ...
    const previousActivity = studentQuizResults.filter((q) => q.updatedAt >= fourWeeksAgo && q.updatedAt < twoWeeksAgo).length;
    const recentScores = studentQuizResults.filter((q) => q.updatedAt >= twoWeeksAgo).map((q) => q.percentage);
    const previousScores = studentQuizResults.filter((q) => q.updatedAt >= fourWeeksAgo && q.updatedAt < twoWeeksAgo).map((q) => q.percentage);
    ...
    else if (recentAvg < previousAvg - 5 || (recentActivity < previousActivity - 2 && previousActivity > 0)) trend = 'declining';
