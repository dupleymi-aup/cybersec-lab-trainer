...
export async function GET(request: NextRequest) {
  ...
  for (const groupName of groupNames) {
    ...
    const groupProgress = progress.filter((p) =>
      groupStudentIds.includes(p.userId),
    );
    const groupQuizAttempts = quizAttempts.filter((q) =>
      groupStudentIds.includes(q.userId),
    );
    const groupQuizResults = quizResults.filter((q) =>
      groupStudentIds.includes(q.userId),
    );

    // Activity timeline (weekly buckets)
    ...
    const variance =
      scores.length > 1
        ? scores.reduce((sum, s) => sum + Math.pow(s - avgScore, 2), 0) /
          scores.length
        : 0;
    const performanceVariance = Math.round(Math.sqrt(variance) * 10) / 10; // std dev
    ...
    const studentScores = groupStudents
      .map((s) => {
        ...
        const sScores = sProgress
          .filter((p) => p.score != null)
          .map((p) => p.score ?? 0);
        return sScores.length > 0
          ? sScores.reduce((a, b) => a + b, 0) / sScores.length
          : 0;
      })
      .sort((a, b) => b - a);

    const topQuartile = studentScores.slice(
      0,
      Math.max(1, Math.floor(studentScores.length / 4)),
    );
    const restScores = studentScores.slice(
      Math.max(1, Math.floor(studentScores.length / 4)),
    );
    const topAvg =
      topQuartile.length > 0
        ? topQuartile.reduce((a, b) => a + b, 0) / topQuartile.length
        : 0;
    const restAvg =
      restScores.length > 0
        ? restScores.reduce((a, b) => a + b, 0) / restScores.length
        : 0;
    const peerInfluenceScore =
      topAvg > 0 && restAvg > 0 ? Math.round((restAvg / topAvg) * 100) : 0;

    // New member integration (avg days from registration to first activity)
    const integrationDays: number[] = [];
    for (const s of groupStudents) {
      const sProgress = groupProgress.filter((p) => p.userId === s.id);
      if (sProgress.length > 0) {
        const firstActivity = new Date(
          Math.min(...sProgress.map((p) => p.updatedAt.getTime())),
        );
        const daysToIntegrate = Math.floor(
          (firstActivity.getTime() - s.createdAt.getTime()) /
            (24 * 60 * 60 * 1000),
        );
    ...
    const newMemberIntegrationDays =
      integrationDays.length > 0
        ? Math.round(
            integrationDays.reduce((a, b) => a + b, 0) / integrationDays.length,
          )
        : 0;
    ...
    const completionRate =
      groupStudents.length > 0
        ? Math.round(
            (groupProgress.filter((p) => p.completed).length /
              (groupStudents.length * 10)) *
              100,
          )
        : 0;
    const quizRate =
      groupStudents.length > 0
        ? Math.min(
            100,
            Math.round((groupQuizResults.length / groupStudents.length) * 20),
          )
        : 0;
    const healthScore = Math.round(
      activityRate * 0.4 + completionRate * 0.35 + quizRate * 0.25,
    );

    // Trend
    const recentWeeks = activityTimeline.slice(-4);
    const olderWeeks = activityTimeline.slice(-8, -4);
    const recentActive = recentWeeks.reduce(
      (sum, w) => sum + w.activeStudents,
      0,
    );
    const olderActive =
      olderWeeks.length > 0
        ? olderWeeks.reduce((sum, w) => sum + w.activeStudents, 0)
        : 0;
    let trend: "improving" | "stable" | "declining";
    if (olderActive > 0 && recentActive > olderActive * 1.1)
      trend = "improving";
    else if (olderActive > 0 && recentActive < olderActive * 0.9)
      trend = "declining";
    else trend = "stable";
  ...
  const allWeeks = new Map<
    string,
    { healthScores: number[]; activeCount: Set<string> }
  >();
  for (const g of groupsData) {
    for (const w of g.activityTimeline) {
      if (!allWeeks.has(w.week))
        allWeeks.set(w.week, { healthScores: [], activeCount: new Set() });
      ...
      totalActive: students.filter((s) => {
        ...
        return (
          ...
          quizAttempts.some(
            (q) =>
              q.userId === s.id &&
              q.attemptedAt >= weekStart &&
              q.attemptedAt < weekEnd,
          )
        );
      }).length,
    }));
