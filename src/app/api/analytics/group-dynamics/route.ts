...
export async function GET(request: NextRequest) {
  ...
  for (const groupName of groupNames) {
    ...
    const weekMap = new Map<string, { activeStudents: Set<string>; modulesCompleted: number; quizAttempts: number }>();
    ...
    const peerInfluenceScore = topAvg > 0 && restAvg > 0
      ? Math.round((restAvg / topAvg) * 100)
      : 0;
    ...
    const newMemberIntegrationDays = integrationDays.length > 0
      ? Math.round(integrationDays.reduce((a, b) => a + b, 0) / integrationDays.length)
      : 0;

    // Health score (composite 0-100)
    const activityRate = activityTimeline.length > 0
      ? Math.round((activityTimeline.reduce((sum, w) => sum + w.activeStudents, 0) / (activityTimeline.length * groupStudents.length)) * 100)
      : 0;
    const completionRate = groupStudents.length > 0
      ...
      : 0;
    const quizRate = groupStudents.length > 0
  ...
  const overallTrends = Array.from(allWeeks.entries())
    ...
    .map(([week, data]) => ({
      ...
      totalActive: students.filter((s) => {
        ...
        return progress.some((p) => p.userId === s.id && p.updatedAt >= weekStart && p.updatedAt < weekEnd) ||
