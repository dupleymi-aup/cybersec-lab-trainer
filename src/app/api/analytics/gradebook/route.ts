...
export async function GET(request: NextRequest) {
  ...
  const students = await prisma.user.findMany({
    where: userFilter,
    select: {
      id: true, fullName: true, email: true, group: true,
    },
  ...
  const studentData = students.map((student) => {
    ...
    // Last active date
    const lastActivityDate = studentProgress.length > 0 || studentQuizResults.length > 0
      ? new Date(Math.max(
    ...
    // Module scores
    const moduleScores: Record<string, { completed: boolean; score: number | null }> = {};
