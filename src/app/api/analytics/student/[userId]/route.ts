...
export async function GET(request: NextRequest, { params }: { params: Promise<{ userId: string }> }) {
  try {
    ...
    for (const [category, cohortData] of cohortCategoryAverages) {
      ...
      const studentRate =
        studentData && studentData.total > 0 ? Math.round((studentData.correct / studentData.total) * 1000) / 10 : 0;
    ...
    recommendations.sort(
      (a, b) =>
        (priorityOrder as Record<string, number>)[a.priority] - (priorityOrder as Record<string, number>)[b.priority],
    );
    ...
    const achievements = [
      ...
      },
      {
        ...
        unlockedAt:
          modulesCompleted >= 1
            ? (progressRecords.find((p: ProgressRow) => p.completed)?.updatedAt.toISOString() ?? null)
            : null,
      },
      {
        id: 'five_modules',
    ...
    logger.error('Student report error:', { error: String(error) });
