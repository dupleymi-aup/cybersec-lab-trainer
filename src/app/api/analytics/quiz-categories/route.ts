...
export async function GET(request: NextRequest) {
  ...
  const categories = Array.from(categoriesMap.entries()).map(
    ([catId, attempts]) => {
      ...
      const avgScore =
        attempts.length > 0
          ? Math.round((correctCount / attempts.length) * 10000) / 100
          : 0;
      const passRate =
        attempts.length > 0
          ? Math.round(
      ...
      const questionStats = Array.from(questionMap.entries()).map(
        ([questionId, qAttempts]) => {
          const qCorrect = qAttempts.filter((a) => a.correct).length;
          const correctRate =
            qAttempts.length > 0
              ? Math.round((qCorrect / qAttempts.length) * 10000) / 100
              : 0;
  ...
  const allQuestionMap = new Map<
    string,
    { attempts: number; correct: number; category: string }
  >();
  for (const attempt of quizAttempts) {
    ...
    if (existing) {
      ...
      allQuestionMap.set(attempt.questionId, {
        attempts: 1,
  ...
  const hardestQuestions = Array.from(allQuestionMap.entries())
    .map(([questionId, data]) => ({
      ...
      correctRate:
        data.attempts > 0
          ? Math.round((data.correct / data.attempts) * 10000) / 100
          : 0,
