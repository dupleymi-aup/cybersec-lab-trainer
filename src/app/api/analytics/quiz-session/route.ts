...
export async function GET(request: NextRequest) {
  ...
  for (const [, session] of sessionMap) {
    ...
    const lastTime =
      session.attempts[session.attempts.length - 1].attemptedAt.getTime();
    ...
    const percentage = Math.round(
      (correctCount / session.attempts.length) * 100,
    );

    // Get quiz result score if available
    const qr = quizResults.find(
      (r) => r.userId === session.userId && r.quizId === session.quizId,
    );
  ...
  for (const s of sessionsWithDuration) {
    if (!categoryTimingMap.has(s.category))
      categoryTimingMap.set(s.category, []);
  ...
  const categoryTiming = Array.from(categoryTimingMap.entries()).map(
    ([cat, durations]) => {
  ...
  const timeVsPerformance = durationBuckets
    ...
    .map((b) => ({
      ...
      avgPercentage:
        Math.round(
          (b.percents.reduce((a, x) => a + x, 0) / b.percents.length) * 10,
        ) / 10,
  ...
  for (const s of sessionsWithDuration) {
    const firstAttempt = sessionsWithDuration.find(
      (a) => a.userId === s.userId && a.quizId === s.quizId,
    );
    if (!firstAttempt) continue;
    // Use the quiz result updatedAt for the hour
    const qr = quizResults.find(
      (r) => r.userId === s.userId && r.quizId === s.quizId,
    );
    if (!qr) continue;
  ...
  const weekdayVsWeekend = [
    {
      ...
      avgPercentage:
        weekdayData.length > 0
          ? Math.round(
              (weekdayData.reduce((a, b) => a + b, 0) / weekdayData.length) *
                10,
            ) / 10
          : 0,
      ...
      avgDuration:
        weekdaySessions.length > 0
          ? Math.round(
              (weekdaySessions.reduce((a, s) => a + s.duration, 0) /
    ...
    },
    {
      ...
      avgPercentage:
        weekendData.length > 0
          ? Math.round(
              (weekendData.reduce((a, b) => a + b, 0) / weekendData.length) *
                10,
            ) / 10
          : 0,
      ...
      avgDuration:
        weekendSessions.length > 0
          ? Math.round(
              (weekendSessions.reduce((a, s) => a + s.duration, 0) /
                weekendSessions.length) *
                10,
            ) / 10
          : 0,
    },
