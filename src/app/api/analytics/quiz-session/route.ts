    ...
    const weekdayVsWeekend = [
      {
        ...
        avgPercentage:
          weekdayData.length > 0
            ? Math.round((weekdayData.reduce((a, b) => a + b, 0) / weekdayData.length) * 10) / 10
            : 0,
        attemptCount: weekdayData.length,
        avgDuration:
          weekdaySessions.length > 0
            ? Math.round((weekdaySessions.reduce((a, s) => a + s.duration, 0) / weekdaySessions.length) * 10) / 10
            : 0,
      },
      {
        ...
        avgPercentage:
          weekendData.length > 0
            ? Math.round((weekendData.reduce((a, b) => a + b, 0) / weekendData.length) * 10) / 10
            : 0,
        attemptCount: weekendData.length,
        avgDuration:
          weekendSessions.length > 0
            ? Math.round((weekendSessions.reduce((a, s) => a + s.duration, 0) / weekendSessions.length) * 10) / 10
            : 0,
      },
    ];
