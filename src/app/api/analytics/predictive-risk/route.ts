...
export async function GET(request: NextRequest) {
  ...
  for (const student of students) {
    ...
    const loginScore = Math.max(
      0,
      Math.min(100, Math.round((1 - loginRatio) * 100)),
    );
    ...
    if (studentQuizzes.length >= 2) {
      ...
      const firstHalf =
        studentQuizzes
          .slice(0, midPoint)
          .reduce((s, q) => s + q.percentage, 0) / midPoint;
      const secondHalf =
        studentQuizzes.slice(midPoint).reduce((s, q) => s + q.percentage, 0) /
    ...
    const completionScore = Math.max(
      0,
      Math.min(100, Math.round((1 - completionRatio) * 100)),
    );

    // Factor 4: Days since last activity (weight: 20%)
    const daysSinceLastLogin = student.lastLoginAt
      ? Math.floor(
          (now.getTime() - new Date(student.lastLoginAt).getTime()) /
            (24 * 60 * 60 * 1000),
        )
      : days;
    const inactivityScore = Math.min(
      100,
      Math.round((daysSinceLastLogin / days) * 100),
    );
    ...
    const quizDeclineRatio =
      olderQuizzes > 0 ? 1 - recentQuizzes / Math.max(olderQuizzes, 1) : 0;
    const quizDeclineScore = Math.max(
      0,
      Math.min(100, Math.round(quizDeclineRatio * 100)),
    );
    ...
    const dropoutProbability =
      Math.round((1 / (1 + Math.exp(-(riskScore - 50) / 15))) * 100) / 100;

    // Recommended interventions
    const interventions: string[] = [];
    if (loginScore >= 60)
      interventions.push(
        "Связаться со студентом и уточнить причины отсутствия",
      );
    if (quizScoreTrend >= 60)
      interventions.push("Предложить дополнительную помощь по квизам");
    if (completionScore >= 60)
      interventions.push("Упростить доступ к следующим модулям");
    if (inactivityScore >= 70)
      interventions.push("Отправить уведомление о прогрессе");
    if (quizDeclineScore >= 60)
      interventions.push("Предложить повторное прохождение квизов");
  ...
  const mediumRisk = studentRisks.filter(
  ...
  const avgRisk =
    studentRisks.length > 0
      ? Math.round(
          studentRisks.reduce((s, r) => s + r.riskScore, 0) /
            studentRisks.length,
        )
      : 0;
