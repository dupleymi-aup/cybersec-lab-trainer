...
export async function GET(request: NextRequest) {
  ...
  if (studentIds.length === 0) {
    return NextResponse.json({
      ...
      weeklyActivity: Array.from({ length: 7 }, (_, i) => ({ day: i, count: 0 })),
      totalActivities: 0,
      mostActiveDay: null,
      mostActiveHour: null,
      streakDays: 0,
  ...
  const allActivities: { date: string; hour: number; dayOfWeek: number; userId: string }[] = [];
  ...
  // Most active day
  const mostActiveDayEntry = dailyActivity.length > 0
