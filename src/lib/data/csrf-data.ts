...
export const attackSteps = [
  ...
  },
  {
    ...
    detail:
      "На evil.com загружается страница с невидимой формой, JavaScript автоматически отправляет POST-запрос.",
    icon: "AlertTriangle",
  ...
  },
  {
    ...
    detail:
      "Сервер проверяет session_id → cookie валиден → выполняет перевод. Деньги переведены злоумышленнику!",
    icon: "AlertTriangle",
...
export const csrfChallenges = [
  {
    ...
    description:
      "Перед вами код формы банковского перевода. Найдите проблему и выберите правильное решение.",
