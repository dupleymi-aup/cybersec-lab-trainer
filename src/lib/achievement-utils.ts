...
export function getAchievementStatus(
  ...
  switch (id) {
    ...
    case 'first-challenge':
      return (challengeStats?.owaspCorrect ?? 0) > 0 || (challengeStats?.authCorrect ?? 0) > 0;
...
export function countUnlockedAchievements(
  ...
  return achievements.filter((a) => getAchievementStatus(a.id, completedModules, quizScores, challengeStats)).length;
}
