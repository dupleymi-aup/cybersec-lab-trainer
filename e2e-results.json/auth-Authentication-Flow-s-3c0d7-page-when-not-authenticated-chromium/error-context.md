...
- main:
  ...
  - img "Animated code terminal": "vulnerability.js 1 // Vulnerable code - find the bug! 2 3 app.post('/login', (req, res) => { 4 const { username, password } = req.body; 5 6 const query = ` 7 SELECT * FROM users 8 WHERE username = '${username}' 9 AND password = '${password}'` 10 11 // ⚠️ SQL Injection vulnerability! 12 db.query(query, (err, result) => { 13 if (result.length > 0) {"
  - paragraph: Модулей
  - paragraph: "8"
  - text: Актуально 2026
  - paragraph: Квизов
