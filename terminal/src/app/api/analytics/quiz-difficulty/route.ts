...
export async function GET(request: NextRequest) {
  ...
  const allAttempts = await prisma.quizAttempt.findMany({
    ...
    select: { userId: true, category: true, difficulty: true, correct: true, attemptedAt: true },
  });
  ...
  const weekMap = new Map<string, { easy: { total: number; correct: number }; medium: { total: number; correct: number }; hard: { total: number; correct: number } }>();
  for (const attempt of attempts) {
