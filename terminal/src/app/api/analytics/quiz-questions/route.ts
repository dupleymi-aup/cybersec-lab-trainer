...
const questionMap = new Map(
  quizQuestions.map((q) => [q.id, { questionText: q.question, category: q.category, difficulty: q.difficulty }])
);
...
export async function GET(request: NextRequest) {
  ...
  const attempts = await prisma.quizAttempt.findMany({
    ...
    select: { questionId: true, correct: true, category: true, difficulty: true },
  });

  // Group by questionId
  const questionStats = new Map<string, { totalAttempts: number; correctCount: number; category: string; difficulty: string }>();
