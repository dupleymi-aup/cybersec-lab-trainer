...
export async function GET(
  ...
  const csv = generateStudentReportCSV(
    ...
    progress.map(p => ({ moduleId: p.moduleId, completed: p.completed, score: p.score })),
    quizResults.map(q => ({ quizId: q.quizId, score: q.score, total: q.total, percentage: q.percentage }))
  );

  const safeName = user.fullName.replace(/[^a-zA-Zа-яА-ЯёЁ0-9\s-]/g, '').trim().replace(/\s+/g, '_') || userId;
