...
import { logAdminAction, AdminActionType } from '@/lib/admin-actions-utils';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const actionTypeMap: Record<BulkAction, AdminActionType> = {
      block: 'user_blocked',
    ...
    // Log each user individually to AdminAction table
    for (const user of targetUsers) {
      await logAdminAction({
        ...
        details: `Admin performed bulk ${action} operation`,
        metadata: { 
          bulkOperation: true, 
          affectedUsers: resultCount,
          newRole: action === 'role_change' ? role : undefined,
        },
        ip,
    ...
    // Legacy audit log for compatibility (single entry for bulk operation)
    try {
