...
export default function QuizCategoryDeepDive({ groupId, days = 30 }: QuizCategoryDeepDiveProps) {
  ...
  const [hardestQuestions, setHardestQuestions] = useState<Array<{ questionId: string; questionText: string; category: string; correctRate: number; attempts: number }>>([]);
              ...
              <div className="space-y-2">
                {hardestQuestions.slice(0, 10).map((q, i) => (
                  <div key={q.questionId} className="flex items-center gap-3 p-2 rounded-lg hover:bg-secondary">
                    ...
                    <span className={`text-xs font-medium ${q.correctRate < 30 ? 'text-red-600' : q.correctRate < 60 ? 'text-amber-600' : 'text-emerald-600'}`}>
                  ...
                  {categories.map((cat) => (
                    <tr key={cat.categoryId} className="border-b border-slate-100 hover:bg-secondary">
                      ...
                      <td className="p-2 text-center">
                        <span className={`font-medium ${cat.avgScore < 40 ? 'text-red-600' : cat.avgScore < 70 ? 'text-amber-600' : 'text-emerald-600'}`}>
                          {Math.round(cat.avgScore)}%
                        </span>
                      </td>
                      <td className="p-2 text-center">
                        <span className={`font-medium ${cat.passRate < 40 ? 'text-red-600' : cat.passRate < 70 ? 'text-amber-600' : 'text-emerald-600'}`}>
