...
interface AnnouncementsResponse {
  announcements: import('@/lib/auth-types').Announcement[];
}
...
function getProficiencyLevel(completedCount: number, totalModules: number, avgScore: number, t: (key: string) => string): { label: string; color: string; bg: string; icon: React.ReactNode } {
  const pct = totalModules > 0 ? completedCount / totalModules : 0;
  if (pct === 0) return { label: t('proficiency.beginner'), color: 'text-muted-foreground', bg: 'bg-muted', icon: <Star size={14} /> };
  if (pct < 0.3) return { label: t('proficiency.novice'), color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-100 dark:bg-emerald-900/40', icon: <Star size={14} /> };
...
export default function Dashboard() {
  ...
  const { data: announcementsData, isLoading: announcementsLoading, error: announcementsError } = useApiQuery<AnnouncementsResponse>(
    ['/api/announcements'],
    { refetchInterval: 60000 },
  );
  const announcements = useMemo(() => announcementsData?.announcements || [], [announcementsData]);

  const { data: deadlinesData, isLoading: deadlinesLoading } = useApiQuery<DeadlinesResponse>(
    ['/api/deadlines/upcoming'],
    { enabled: !!user },
  );
  const upcomingDeadlines = useMemo(() => deadlinesData?.upcoming || [], [deadlinesData]);
  ...
  const { data: adminSummary } = useQuery<AdminSummary>({
    queryKey: ['analytics', 'admin-summary', ''] as const,
    queryFn: () => analyticsApi.getAdminSummary(''),
    enabled: user?.role === 'teacher' || user?.role === 'admin',
  });

  const teacherStats = adminSummary?.current ? {
    totalStudents: adminSummary.current.totalStudents || 0,
    activeStudents: adminSummary.current.activeStudents || 0,
    avgModules: Math.round(adminSummary.current.avgCompletionRate || 0),
    atRiskCount: (adminSummary as unknown as { atRiskCount?: number })?.atRiskCount || 0,
  } : null;
  ...
  const activityTimeline = useMemo(() => {
    ...
    for (const [quizId, ts] of Object.entries(quizTimestamps)) {
      events.push({ date: new Date(ts), type: 'quiz' as const, label: `${t('dashboard.quizPrefix')} ${quizId}` });
    }
  ...
  const recommendation = useMemo(() => {
    const buildRecommendation = () => {
      if (user?.role === 'admin' && completedModules.length === 0) {
        return { text: t('recommendations.adminDefault'), page: 'admin-panel' as PageType };
      }
      if (user?.role === 'teacher' && completedModules.length === 0) {
        return { text: t('recommendations.teacherDefault'), page: 'teacher-panel' as PageType };
      }

      if (streakData.current >= 7) {
        return { text: t('recommendations.greatStreak', { count: streakData.current }), page: 'achievements' as PageType };
      }
      if (streakData.current === 0 && streakData.daysSinceLast >= 3) {
        const nextModule = modules.find((m) => !completedModules.includes(m.id));
        return { text: t('recommendations.inactiveDays', { count: streakData.daysSinceLast }), page: (nextModule?.id || 'quiz') as PageType };
      }
      ...
      if (!completedModules.includes('xss')) {
        return { text: t('recommendations.tryXss'), page: 'xss' as PageType };
      }
      if (!completedModules.includes('csrf')) {
        return { text: t('recommendations.tryCsrf'), page: 'csrf' as PageType };
      }
      if (!completedModules.includes('security-headers')) {
        return { text: t('recommendations.tryHeaders'), page: 'security-headers' as PageType };
      }
      ...
      if (!completedModules.includes('secure-coding')) {
        return { text: t('recommendations.tryCoding'), page: 'secure-coding' as PageType };
      }
      ...
      if (!completedModules.includes('api-security')) {
        return { text: t('recommendations.tryApi'), page: 'api-security' as PageType };
      }
  ...
  const quickActions = [
    ...(user?.role === 'admin' || user?.role === 'teacher'
      ? [{ label: t('sidebar.adminPanel'), page: 'admin-panel' as PageType, icon: Shield, color: 'bg-red-100 text-red-600' }]
      : []),
    ...(user?.role === 'teacher' || user?.role === 'admin'
      ? [{ label: t('sidebar.teacherPanel'), page: 'teacher-panel' as PageType, icon: UserCheck, color: 'bg-amber-100 text-amber-600' }]
      : []),
  ];
    ...
    <div className="space-y-6">
      ...
      {announcementsError && !announcementsLoading && (
        <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-600">
          {t('dashboard.failedToLoadAnnouncements')}
        </div>
      )}
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3 flex-1">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {ann.priority === 'high' && (
                          <Badge className="bg-red-100 text-red-700 border-0 text-[10px]">{t('dashboard.important')}</Badge>
                        )}
                      </div>
                  ...
                  <button
                    ...
                    aria-label={`${t('dashboard.hideAnnouncement')}: ${ann.title}`}
                  >
                    <X size={16} />
                  </button>
                </div>
      ...
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-violet-600 via-violet-700 to-emerald-600 text-white p-6 md:p-10 shadow-xl shadow-violet-500/20"
      >
        ...
        <motion.div
          ...
          className="absolute bottom-0 left-0 w-48 h-48 bg-emerald-400/10 rounded-full translate-y-1/2 -translate-x-1/2 blur-3xl" 
        />
        ...
        <div className="relative z-10">
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            >
              <Badge className={`${proficiency.bg} ${proficiency.color} border-0 backdrop-blur-sm`}>
          ...
          >
            {user?.fullName ? t('dashboard.welcomeWithName', { name: user.fullName.split(' ')[0] }) : t('dashboard.welcome') + '!'}
          </motion.h1>
          ...
          <motion.p 
            ...
            className="text-sm md:text-base text-white/80 max-w-2xl leading-relaxed"
          >
          ...
          >
            <motion.div 
              ...
              className="flex items-center gap-2 text-sm bg-white/10 backdrop-blur-sm px-3 py-1.5 rounded-lg"
            >
              <BookOpen size={16} className="text-emerald-300" />
              <span className="text-white/90">{t('dashboard.modulesCompleted', { count: totalModules })}</span>
            </motion.div>
            <motion.div 
              whileHover={{ scale: 1.05 }}
              className="flex items-center gap-2 text-sm bg-white/10 backdrop-blur-sm px-3 py-1.5 rounded-lg"
            >
              ...
              <span className="text-white/90">{t('dashboard.progressPct', { pct: totalProgress })}</span>
            </motion.div>
            <motion.div 
              whileHover={{ scale: 1.05 }}
              className="flex items-center gap-2 text-sm bg-white/10 backdrop-blur-sm px-3 py-1.5 rounded-lg"
            >
              ...
              <span className="text-white/90">
                {avgQuizScore > 0 ? t('dashboard.avgScoreValue', { score: avgQuizScore }) : t('dashboard.takeQuiz')}
              </span>
            </motion.div>
            <motion.div 
              whileHover={{ scale: 1.05 }}
            ...
            >
              ...
              <span className="text-white/90">{t('dashboard.achievementsCount', { count: unlockedCount })}</span>
            </motion.div>
          </motion.div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4">
        {[
          { label: t('dashboard.modules'), value: `${completedCount}/${totalModules}`, color: 'from-emerald-500 to-emerald-600', icon: BookOpen },
          { label: t('dashboard.quizzes'), value: `${Object.keys(quizScores).length}/${quizCategories.length}`, color: 'from-amber-500 to-amber-600', icon: HelpCircle },
          { label: t('dashboard.avgScore'), value: `${avgQuizScore}%`, color: 'from-sky-500 to-sky-600', icon: Target },
          { label: t('dashboard.achievements'), value: `${unlockedCount}/${achievements.length}`, color: 'from-violet-500 to-violet-600', icon: Trophy },
          { label: streakData.current > 0 ? t('dashboard.streak') : t('dashboard.noStreak'), value: streakData.current > 0 ? `${streakData.current} ${t('dashboard.daysShort')}` : '—', color: streakData.current > 0 ? 'from-orange-500 to-orange-600' : 'from-slate-400 to-slate-500', icon: Flame, tooltip: streakData.longest > 0 ? t('dashboard.bestStreak', { count: streakData.longest }) : undefined },
          { label: t('dashboard.level'), value: proficiency.label, color: 'from-indigo-500 to-indigo-600', icon: Star },
        ].map((stat, i) => {
          ...
          return (
            ...
            >
              <Card className="border-border/50 shadow-sm bg-card hover:shadow-lg hover:shadow-violet-500/10 transition-all duration-300 overflow-hidden relative group">
                ...
                <CardContent className="p-4 text-center relative z-10">
                  ...
                  <p className={`text-xl md:text-2xl font-bold bg-gradient-to-r ${stat.color} bg-clip-text text-transparent`}>{stat.value}</p>
                  {stat.tooltip && streakData.longest > 0 && (
                    <p className="text-[10px] text-orange-500 mt-0.5 font-medium">{stat.tooltip}</p>
                  )}
      ...
      {(user?.role === 'teacher' || user?.role === 'admin') && !teacherStats && !adminSummary && (
      ...
      {teacherStats && (
        ...
        >
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-semibold text-amber-800">{t('dashboard.classOverview')}</span>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div>
              <p className="text-xl font-bold text-amber-700">{teacherStats.totalStudents}</p>
              <p className="text-xs text-amber-600/80">{t('dashboard.totalStudents')}</p>
            </div>
            <div>
              <p className="text-xl font-bold text-emerald-700">{teacherStats.activeStudents}</p>
              <p className="text-xs text-emerald-600/80">{t('dashboard.activeStudents')}</p>
            </div>
            <div>
              <p className="text-xl font-bold text-violet-700">{teacherStats.avgModules}%</p>
              <p className="text-xs text-violet-600/80">{t('dashboard.avgCompletion')}</p>
            </div>
            <div>
              <p className="text-xl font-bold text-red-600">{teacherStats.atRiskCount}</p>
              <p className="text-xs text-red-500/80">{t('dashboard.atRisk')}</p>
            </div>
          </div>
        </motion.div>
      )}
          ...
          <Card className="bg-gradient-to-r from-emerald-50 to-teal-50 border-emerald-200 cursor-pointer hover:shadow-md transition-shadow h-full"
            ...
            <CardContent className="p-5 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3 flex-1">
                ...
                <div>
                  <p className="text-xs text-emerald-600 font-medium">{t('dashboard.recommendation')}</p>
            ...
            <Card className="border-amber-200 bg-gradient-to-r from-amber-50 to-yellow-50 h-full">
              <CardContent className="p-5 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-amber-600">{t('dashboard.nextAchievement')}</p>
              ...
              <div className="space-y-3">
                {upcomingDeadlines.map((deadline) => {
                  ...
                  const urgencyBadge = deadline.isOverdue
                    ? <Badge className="bg-red-100 text-red-700 border-0 text-[10px]">{t('dashboard.overdue')}</Badge>
                    : deadline.daysLeft <= 1
                      ? <Badge className="bg-red-100 text-red-700 border-0 text-[10px]">{deadline.daysLeft === 0 ? t('dashboard.today') : t('dashboard.tomorrow')}</Badge>
                      : <Badge className="bg-orange-100 text-orange-700 border-0 text-[10px]">{deadline.daysLeft} {t('dashboard.daysShort')}</Badge>;

                  const scopeLabel = deadline.scope === 'course'
                    ? t('dashboard.course')
                    : deadline.scope === 'module'
                      ? modules.find(m => m.id === deadline.scopeId)?.title || deadline.scopeId
                      : `${t('dashboard.quizPrefix')} ${deadline.scopeId}`;
      ...
      {/* Module Cards */}
      <div>
        <h2 className="text-lg md:text-xl font-bold mb-4">{t('dashboard.learningModules')}</h2>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {modules.map((mod, i) => {
                    ...
                    <div className="flex">
                      ...
                      <div className="flex-1 p-4">
                        ...
                        <div className="flex items-center gap-2 mt-3 flex-wrap">
                          ...
                          <span className="text-[11px] text-slate-400">{mod.lessons} {t('dashboard.lessons')}</span>
                          {isCompleted && (
                            <Badge className="bg-emerald-100 text-emerald-700 border-0 text-[10px]">{t('dashboard.passed')}</Badge>
                          )}
                        </div>
                      </div>
                    </div>
