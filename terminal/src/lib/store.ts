...
function fetchWithTimeout(input: RequestInfo | URL, init?: RequestInit, timeoutMs = 15000) {
  ...
  const combined = { ...init, signal: controller.signal };
  return fetch(input, combined).finally(() => clearTimeout(timeoutId));
}
...
const apiClient = {
  ...
  async batchSave(progress: unknown[], quizResults: unknown[]) {
    const response = await fetchWithTimeout('/api/progress/batch', {
      method: 'POST',
      headers: await getAuthHeaders(),
