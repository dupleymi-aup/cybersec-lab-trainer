...
app.post('/transfer', (req, res) => {
  ...
  db.execute(
    ...
    correctIndex: 1,
    explanation: 'Хотя сервер проверяет session.userId, нет защиты от CSRF. Злоумышленник может создать форму на своём сайте, и при посещении её пользователем (у которого уже есть сессия bank.com) перевод будет выполнен. Браузер автоматически отправит куки bank.com с запросом.',
  },
  {
    id: 2,
    title: 'Выбери защиту: Email с кнопкой',
    description: 'Пользователь получил email от «банка» с кнопкой «Подтвердить вход». При нажатии она автоматически отправляет POST-запрос к bank.com/login-confirm.',
    ...
    correctIndex: 1,
    explanation: 'CSRF-токен не позволит злоумышленнику создать рабочую форму (он не знает токен), а SameSite=Strict заблокирует отправку кук с кросс-сайтового запроса из email клиента.',
  },
  {
    id: 3,
...
app.put('/api/profile', authenticate, (req, res) => {
  ...
  res.json({ ok: true });
    ...
    correctIndex: 1,
    explanation: 'Эндпоинт использует кастомный заголовок Authorization (через JWT). Браузер не может автоматически добавить этот заголовок в кросс-доменный запрос без CORS preflight. Это один из способов защиты от CSRF — использование кастомных заголовков вместо кук.',
  },
  {
    id: 4,
    title: 'Реальный сценарий: Изменение пароля',
    description: 'Злоумышленник разместил на evil.com страницу с изображением: <img src="https://bank.com/change-password?new=Hacked123">. Что произойдёт?',
    ...
    correctIndex: 1,
    explanation: 'Атака сработает ТОЛЬКО если банк использует GET-запрос для смены пароля — это нарушение HTTP-спецификации (GET должен быть идемпотентным). Правильно реализованный банк использует POST/PUT с CSRF-токеном. Тег <img> делает GET-запрос, но не POST.',
  },
  {
    id: 5,
...
app.post('/delete-account', (req, res) => {
  ...
  res.json({ ok: true });
    ...
    correctIndex: 0,
    explanation: 'CSRF middleware (например, csurf) генерирует уникальный токен для каждой сессии. Форма должна содержать скрытое поле <input type="hidden" name="_csrf" value="{{csrfToken}}">. Сервер проверяет токен из формы против токена в сессии — злоумышленник не может получить токен из-за Same-Origin Policy.',
  },
];
...
export const realWorldExamples = [
  {
    year: '2019',
    company: 'TikTok',
    description: 'Исследователи обнаружили CSRF-уязвимость в TikTok, которая позволяла изменить email аккаунта, подписаться на пользователя и поставить лайк через вредоносный сайт. Уязвимость была в API-эндпоинтах, не проверяющих CSRF-токены.',
  ...
  },
  {
    ...
    description: 'В ранних версиях Gmail CSRF-токены передавались в URL-параметрах. При клике на внешнюю ссылку токен попадал в Referer-заголовок и мог быть прочитан сторонним сервером.',
  ...
  },
  {
    ...
    description: 'Исследователь обнаружил CSRF-уязвимость в веб-интерфейсе Tesla, которая позволяла удалённо открыть дверь автомобиля. Атакующий мог создать вредоносную страницу, которая отправляла запрос к API Tesla.',
