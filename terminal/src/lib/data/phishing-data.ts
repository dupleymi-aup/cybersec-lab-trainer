...
<p>С уважением,<br>Служба безопасности Сбербанка</p>
    ...
    isPhishing: true,
    indicators: [
      { id: 'urgency-1', type: 'urgency', severity: 'high', title: 'Искусственная срочность', description: '«Будет заблокирован в течение 24 часов» — классический приём создания паники, чтобы жертва действовала не думая.' },
      { id: 'sender-1', type: 'sender', severity: 'critical', title: 'Подозрительный домен отправителя', description: 'Домен sberbank-security.com — не официальный домен Сбербанка (sberbank.ru). Злоумышленники регистрируют похожие домены.' },
      { id: 'link-1', type: 'link', severity: 'critical', title: 'Фишинговая ссылка', description: 'Ссылка ведёт на sberbank-verify.ru — не официальный сайт Сбербанка. Официальный сайт: online.sberbank.ru.' },
      { id: 'header-1', type: 'header', severity: 'critical', title: 'Подозрительный Return-Path', description: 'Return-Path: noreply@phishing-kit.ru — явно указывает на использование фишинг-набора.' },
      { id: 'header-2', type: 'header', severity: 'high', title: 'Подозрительный X-Mailer', description: 'PHPMailer 5.2.23 — массовая рассылка через скрипт, а не через корпоративную почту.' },
      { id: 'header-3', type: 'header', severity: 'medium', title: 'IP отправителя из динамического диапазона', description: 'Received from dynamic.isp.com — легитимные банки отправляют с фиксированных серверов.' },
    ],
    explanation: 'Это классическое фишинговое письмо. Злоумышленник создал домен, похожий на официальный, использует искусственную срочность и отправляет через массовый скрипт. Return-Path явно указывает на фишинг-набор.',
  },
  {
    id: 2,
    difficulty: 'medium',
...
<p>С уважением,<br>Команда Google</p>
    ...
    isPhishing: false,
    indicators: [],
    explanation: 'Это легитимное письмо от Google. DKIM-подпись пройдена, SPF Pass, отправитель с официального IP Google (142.250.80.33), ссылка ведёт на policies.google.com — официальный домен.',
  },
  {
    id: 3,
    difficulty: 'hard',
...
<p>Best regards,<br>Microsoft Billing Department</p>
    ...
    isPhishing: true,
    indicators: [
      { id: 'sender-3a', type: 'sender', severity: 'critical', title: 'Омографическая атака (typosquatting)', description: 'Домен micr0soft.com — вместо буквы "o" использован ноль "0". Это классическая омографическая атака. Официальный домен: microsoft.com.' },
      { id: 'link-3a', type: 'link', severity: 'critical', title: 'Ссылка на поддельный домен', description: 'Ссылка ведёт на micr0soft.com (с нулём), а не microsoft.com. Злоумышленник контролирует весь домен.' },
      { id: 'header-3a', type: 'header', severity: 'high', title: 'DKIM подписан поддельным доменом', description: 'DKIM прошёл, но для домена micr0soft.com — это не значит что домен легитимный. Злоумышленник настроил DKIM для своего домена.' },
      { id: 'content-3a', type: 'content', severity: 'medium', title: 'Вложение/ссылка на «инвойс»', description: 'Ссылка на PDF может вести на вредоносный файл или страницу загрузки малвари.' },
    ],
    explanation: 'Это sophisticated фишинговое письмо с омографической атакой. Все технические проверки (DKIM, SPF) пройдены, но для поддельного домена micr0soft.com (с нулём вместо "o"). Такие атаки особенно опасны, так выглядят технически корректными.',
  },
  {
    id: 4,
    difficulty: 'easy',
...
<p><i>* Осталось 2 часа до окончания акции!</i></p>
    ...
    isPhishing: true,
    indicators: [
      { id: 'urgency-4a', type: 'urgency', severity: 'high', title: 'Фейковая срочность', description: '«Осталось 2 часа» — попытка заставить действовать быстро, без размышлений.' },
      { id: 'sender-4a', type: 'sender', severity: 'critical', title: 'Подозрительный домен', description: 'free-iphone-2024.xyz — явно мошеннический домен. Apple не проводит розыгрыши через email.' },
      { id: 'content-4a', type: 'content', severity: 'high', title: 'Слишком хорошо, чтобы быть правдой', description: '«Вы выиграли iPhone» — классическая схема. Если вы не участвовали в розыгрыше, это мошенничество.' },
      { id: 'header-4a', type: 'header', severity: 'high', title: 'Массовая рассылка', description: 'X-Mailer: BulkMailer Pro 3.1 — софт для спам-рассылок. Apple использует корпоративные почтовые системы.' },
      { id: 'header-4b', type: 'header', severity: 'medium', title: 'Отправка в 3 часа ночи', description: 'Письмо отправлено в 03:15 — массовые рассылки часто запускают ночью.' },
    ],
    explanation: 'Очевидное фишинговое письмо. Розыгрыши Apple через email не проводятся. Домен-однодневка, BulkMailer для рассылки, агрессивный дизайн — всё указывает на мошенничество.',
  },
  {
    id: 5,
    difficulty: 'medium',
...
<p>Спасибо за покупку!</p>
    ...
    isPhishing: false,
    indicators: [],
    explanation: 'Легитимное письмо от Wildberries. DKIM, SPF и DMARC пройдены для официального домена wildberries.ru. Ссылка ведёт на www.wildberries.ru. Нет признаков фишинга.',
  },
  {
    id: 6,
    difficulty: 'hard',
...
<p style="font-size:12px;color:#666;">Netflix, Inc. | 100 Winchester Circle | Los Gatos, CA 95032</p>
    ...
    isPhishing: true,
    indicators: [
      { id: 'sender-6a', type: 'sender', severity: 'critical', title: 'Поддомен-имперсонация', description: 'Домен netflix.com.ru — это НЕ netflix.com. Это российский домен com.ru с поддоменом netflix. Официальный домен Netflix: netflix.com.' },
      { id: 'link-6a', type: 'link', severity: 'critical', title: 'Ссылка на поддельный сайт', description: 'Ссылка ведёт на netflix.com.ru/account/billing/update — поддельная страница для сбора данных карт.' },
      { id: 'urgency-6a', type: 'urgency', severity: 'high', title: 'Дедлайн в 48 часов', description: '«В течение 48 часов аккаунт будет приостановлен» — искусственная срочность.' },
      { id: 'header-6a', type: 'header', severity: 'medium', title: 'Использование SendGrid', description: 'Netflix использует собственную инфраструктуру, а не SendGrid для security-уведомлений.' },
      { id: 'content-6a', type: 'content', severity: 'medium', title: 'Логотип с поддельного домена', description: 'Изображение загружается с netflix.com.ru — злоумышленник загрузил копию логотипа на свой сервер.' },
    ],
    explanation: 'Sophisticated фишинг с использованием домена-подражателя. netflix.com.ru выглядит похоже на netflix.com, но это完全不同的 домен. DKIM/SPF пройдены, но для поддельного домена. Это один из самых опасных типов фишинга.',
  },
];
...
export const phishingEducationContent = {
  whatIsPhishing: {
    ...
    description: 'Фишинг — это вид интернет-мошенничества, целью которого является получение доступа к конфиденциальным данным пользователей: логинам, паролям, номерам банковских карт и другой личной информации.',
  },
  commonTypes: [
    {
      type: 'Email-фишинг',
    ...
    {
      type: 'Spear phishing',
      description: 'Целевая атака на конкретного человека или организацию. Письмо персонализировано и содержит информацию о жертве.',
      icon: 'Target',
    },
    {
      type: 'Whaling',
      description: 'Атака на топ-менеджеров и руководителей. Особенно тщательно подготовленные письма с большими суммами.',
      icon: 'Crown',
    },
    {
      type: 'Clone phishing',
      description: 'Копирование легитимного письма с заменой ссылки на фишинговую. Отправляется как «обновлённая версия».',
      icon: 'Copy',
    },
  ],
  howToSpot: [
