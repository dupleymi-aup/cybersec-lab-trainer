...
export function getLevel(xp: number): { level: number; rank: UserRank } {
  ...
  return { level: Math.min(level, MAX_LEVEL), rank: getRank(Math.min(level, MAX_LEVEL)) };
}

export function getXpForNextLevel(xp: number): { current: number; needed: number; nextLevel: number } {
