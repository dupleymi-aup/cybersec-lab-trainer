 ...
 *
 * @deprecated Use `useApiQuery` from `@/hooks/use-api` instead, which provides
 * automatic caching, background refetching, deduplication, and more.
 * Or use the dedicated hooks from `@/hooks/use-analytics-query`.
 *
    ...
    fetch(url, { signal: controller.signal })
      ...
      .catch((err) => {
        if (!cancelled) {
          ...
          if (process.env.NODE_ENV === 'development') logger.error(`useAnalyticsFetch: Failed to fetch ${endpoint}:`, { error: String(err) });
 ...
 *
 * @deprecated Use `useQuery` from `@tanstack/react-query` directly instead, which
 * provides automatic caching, background refetching, deduplication, and more.
 *
 ...
 *
 * @deprecated Use `useApiMutation` from `@/hooks/use-api` instead, which provides
 * automatic cache invalidation, optimistic updates, and CSRF protection.
 *
